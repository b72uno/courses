...
def update_rover(Rover, data):
    ...
    # Get the current image from the center of the rover
...
def create_output_images(Rover):
    ...
    # If there are, step through the known sample positions
    # to confirm if detections are real
    samples_located = 0
    ...
    # First get the total number of pixels in the navigable zero terrain map
    tot_nav_pix = np.float(len((plotmap[:,:,2].nonzero()[0])))
    # Then figure out how many of those correspond to ground truth pixels
    good_nav_pix = np.float(len(((plotmap[:,:,2] > 0) & (Rover.ground_truth[:,:,1] > 0)).nonzero()[0]))
    # How many do not correspond to the ground truth pixels
    ...
    # Calculate percentage of ground truth map that has been successfully found
    ...
    # found to be navigable terain
    if tot_nav_pix > 0:
        fidelity = round(100*good_nav_pix/(tot_nav_pix), 1)
    else:
        fidelity = 0

    # Flip the map for plotting so that the y axis points upwards in the display
    map_add = np.flipud(map_add).astype(np.float32)
    # Add some text about map and rock sample detection results
    cv2.putText(map_add,"Mode: "+str(Rover.mode), (100, 10),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    cv2.putText(map_add, str(np.round(Rover.ddata, 3)), (120, 25),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
